...
export async function runSeed() {
  ...
  if (existingCount > 0) {
    say('Tasting sessions already present — skipping session seed (users still upserted).');
    say('Default password for every seeded account: ' + DEFAULT_PASSWORD);
    return log.join('\n');
  }

  say('Seeding tasting sessions...');
  ...
  say('Done. Default password for every seeded account: ' + DEFAULT_PASSWORD);
  return log.join('\n');
}
